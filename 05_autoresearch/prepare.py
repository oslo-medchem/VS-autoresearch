#!/usr/bin/env python3
"""
VS Autoresearch – Fixed infrastructure (never modified by agent).
...
LABELS_SRC = REPO_ROOT / "03_library_preparation" / "library_labels.csv"
LIBRARY_NAIVE_SRC = REPO_ROOT / "03_library_preparation" / "naive" / "pdbqt"
LIBRARY_SKILL_SRC = REPO_ROOT / "03_library_preparation" / "skill" / "pdbqt"
RECEPTOR_NAIVE_SRC = REPO_ROOT / "04_receptor_preparation" / "receptor_naive.pdbqt"
RECEPTOR_SKILL_SRC = REPO_ROOT / "04_receptor_preparation" / "receptor_skill.pdbqt"

# Cache on fast local storage (configurable via environment variable)
CACHE_DIR = Path(os.environ.get("VS_CACHE_DIR", str(REPO_ROOT / "05_autoresearch" / "cache")))
...
# FPR2 binding site center (from 7T6S cryo-EM structure)
BOX_CENTER = (81.893, 115.414, 96.832)


# ── Cache setup ──────────────────────────────────────────────────────────────

def setup_cache():
    """Copy receptors + labels to cache, symlink PDBQT libraries."""
    ...
    # Copy PDBQT libraries to cache (avoids slow network/FUSE I/O)
    for src, dst in [(LIBRARY_NAIVE_SRC, LIBRARY_NAIVE),
                     (LIBRARY_SKILL_SRC, LIBRARY_SKILL)]:
        if not dst.exists():
            print(f"  Copying {src.name} → {dst} (this may take a few minutes)...")
...
if __name__ == "__main__":
    print("=== VS Autoresearch Cache Setup ===")
    setup_cache()
